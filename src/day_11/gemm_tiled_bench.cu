...
torch::Tensor gemm_tiled_forward(torch::Tensor A, torch::Tensor B, float scale) {
    TORCH_CHECK(A.device().is_cuda() , "A must be a cuda tensor");
    TORCH_CHECK(B.device().is_cuda() , "A must be a cuda tensor");
    TORCH_CHECK(A.dtype() == torch::kFloat32, "A must be float32");
    TORCH_CHECK(B.dtype() == torch::kFloat32, "B must be float32");
    TORCH_CHECK(A.size(1) == B.size(0), "A's columns should match B's rows for matmul.");

    int M = A.size(0);
    int K = A.size(1);
    int N = B.size(1);

    auto C = torch::zeros({M, N}, torch::TensorOptions().device(A.device()).dtype(A.dtype()));
    ...
    gemm_tiled<float, TILE_WIDTH><<<grid_dim, block_dim>>>(A.data_ptr<float>(), B.data_ptr<float>(), C.data_ptr<float>(), M, K, N, scale);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("matmul", &gemm_tiled_forward, "Tiled GEMM CUDA forward pass");
