...
template<typename Element>
cutlass::Status run_gqa_looped(
    int B, int N, int G, int L, int D,
    Element const* Q,
    Element const* K,
    Element const* V,
    Element * Scores,
  ...
  // Strides (in elements)
  int64_t strideQ = int64_t(N) * L * D;   // Q[b] block
  int64_t strideK = int64_t(G) * L * D;   // K[b] block
  int64_t strideS = int64_t(N) * L * L;   // Scores[b] block
  int64_t strideV = int64_t(G) * L * D;   // V[b] block
  int64_t strideO = int64_t(N) * L * D;   // Output[b] block
  int64_t group_size = int64_t(L) * D;    // group size
  for (int b = 0; b < B; ++b) {
    // Base pointers for this batch
    auto ptrQ = Q + b * strideQ;
    auto ptrK = K + b * strideK;
    ...
    // Softmax(scores) × V → Output
    {
      typename GemmVO::Arguments args(
        { L, D, L },     // M=L (seq len), N=D, K=L
        { ptrS, L }, L*L,     // each head’s softmax L×L
        { ptrV, D }, group_size,       // reuse same V[b] for all N
        { ptrO, D }, D,
        { ptrO, D }, D,
        { Element(1), Element(0) },
        N
