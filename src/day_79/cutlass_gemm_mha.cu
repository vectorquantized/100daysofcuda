...
int main(int argc, char* argv[]) {
    ...
    // Shape of A/Q: (B, N, L, D)
    // Shape of B/K: (B, N, L, D)
    ...
    int count_A = batch_count * num_heads * lda * D;
    int count_B = batch_count * num_heads * ldb * D;
    int count_C = batch_count * num_heads * ldc * L;
    int count_V = batch_count * num_heads * ldv * D;
    ...
    auto init_A = [kRange, num_heads, lda, D] (int batch, int head, int row, int col) -> float {
        return static_cast<float>((batch * num_heads * lda * D + head * lda * D + col * lda + row) % kRange);
    };
    
    auto init_B = [kRange, num_heads, ldb, D](int batch, int head, int row, int col) -> float {
        return static_cast<float>((batch * num_heads * ldb * D + head * ldb * D + col * ldb + row) % kRange);
    };
    
    auto init_C = [](int batch, int head, int row, int col) -> float {
        return 1.0f;
    };

    auto init_V = [kRange, num_heads, ldv, D](int batch, int head, int row, int col) -> float {
        return static_cast<float>((batch * num_heads * ldv * D + head * ldv * D + col * ldv + row) % kRange);
    };
    
    cpu_utils::initialize_batched_multi_headed_matrices_col_major(host_A.data(), batch_count * num_heads, num_heads, L, D, lda, lda * D, init_A);
    cpu_utils::initialize_batched_multi_headed_matrices_col_major(host_B.data(), batch_count * num_heads, num_heads, L, D, ldb, ldb * D, init_B);
    cpu_utils::initialize_batched_multi_headed_matrices_col_major(host_C.data(), batch_count * num_heads, num_heads, L, L, ldc, ldc * L, init_C);
    cpu_utils::initialize_batched_multi_headed_matrices_col_major(host_V.data(), batch_count * num_heads, num_heads, L, D, ldv, ldv * D, init_V);
