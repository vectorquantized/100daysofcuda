...
torch::Tensor batched_conv2d_forward(torch::Tensor input, torch::Tensor kernel, torch::Tensor output) {
    ...
    TORCH_CHECK(kernel.ndimension() == 4, "Kernel must be 4D tensor (C_out, C,Kh,Kw)");
    TORCH_CHECK(input.size(1) == kernel.size(1), "Channel dimensions must match");
    ...
    size_t shared_mem_size = sizeof(float) * in_channels * in_tile_width * in_tile_width;
    
    conv2d_tiled_channel_in_out<float><<<grid_dim, block_dim, shared_mem_size>>>(
